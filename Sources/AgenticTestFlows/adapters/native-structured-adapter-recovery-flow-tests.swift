...
private struct NativeStructuredRecoveryFixtureInference: Inference {
    @JSONSchema
    struct Input: Source {
        let value: String
    }
    ...
    -> [TestFlowDiagnostic]
{
    ...
    let state = NativeStructuredRecoveryFixtureState()
    let catalog = try InferenceAdapterCatalog(
        adapters: [
            adapter,
        ]
    )
    ...
    let result = try await NativeStructuredRecoveryFixtureInference.execute(
        using: executor,
        input: .init(
            value: "fixture"
        ),
